...
def start_training(model, device, trainloader, testloader, config, optimizer_name = "Adam", scheduler_name = "OneCycle", criterion_name = "CrossEntropyLoss", lambda_l1 = 0, epochs = 100):
    ...
    if (scheduler_name == "OneCycle"):
        scheduler = OneCycleLR(optimizer, max_lr = config['standard_lr'], epochs = epochs, steps_per_epoch = len(trainloader), pct_start = config['oneCycle_pct_start'], div_factor = 10, three_phase = False, anneal_strategy = 'linear', final_div_factor = 50)
